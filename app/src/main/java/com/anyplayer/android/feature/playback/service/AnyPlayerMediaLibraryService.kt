            ...
            object : MediaLibrarySession.Callback {
                override fun onConnect(
                    ...
                    if (!projectionControllerGuard.isTrustedController(controller)) {
                        // Log-only, not enforced: rejecting here would also block debug-build
                        // controllers (e.g. adb/local test harnesses) that don't match the
                        // trusted-package allowlist or count as a system app.
                        CompatLog.w(
                            TAG,
